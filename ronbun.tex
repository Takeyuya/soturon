...
本研究では，提案システムを実装し，DIDの生成・登録，IoTデータのIPFSへの保存，
CIDおよびDIDのブロックチェーンへの記録，VCの発行・署名・検証に至る一連の処理について機能動作試験を行った．
また，IPFSのデータアップロード処理，ブロックチェーンへのCID記録処理，VCの発行及び検証処理に要する時間を計測し，
...
分散環境において検証可能とするIoTデータ管理基盤を構築し，その実装可能性および性能特性を示した．
...
これらの課題を解決するためのアプローチとして，近年ではブロックチェーン技術とIoTを組み合わせたデータ管理手法が注目されている．
ブロックチェーンは，改ざん耐性や透明性を備えた分散型台帳技術であり，IoTデータの真正性検証やアクセス制御，データ履歴の追跡などへの応用が検討されている．
特に，IoTデータのハッシュ値やメタ情報をブロックチェーン上に記録することで，データ改ざんの検知や信頼性の担保を実現する研究が進められている．

一方で，ブロックチェーンは取引データを全ノードで共有・保持する特性を持つため，
...
このような背景のもと，ブロックチェーンを用いたIoTデータ管理およびアクセス制御に関する研究が数多く報告されている．
...
十分に確立されているとは言い難い．
本研究は，この課題に着目し，データ管理と主体識別を同時に満たす分散型IoTデータ管理基盤の構築を目指す点に特徴がある．
...
Decentralized Identifier \cite{cite8}（以下DID）はこの自己主権型IDを実現するための標準技術であり，
ブロックチェーンなどの改ざん耐性を持つ基盤と組み合わせることで，第三者により検証可能な主体識別を実現できる．

本研究では，これらの背景を踏まえ，
IPFS，ブロックチェーン，およびDID/VCを統合した分散型IoTデータ管理システムを提案する．
提案システムでは，
\begin{itemize}
...
第4章では，提案する分散型IoTデータ管理システムの構成および処理フローについて詳述する．
