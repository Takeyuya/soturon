...
\def\@gyear{2025}
\def\@thesis{卒業論文}
\def\@title{DID/VCに基づく分散型IoTデータ管理システムの実装と性能評価}
\def\@date{2026年2月10日}
...
\section{関連研究}
IoTデータ管理に関する研究では，デバイス数の増加に伴うスケーラビリティの問題や，データの完全性・真正性をいかに保証するかが重要な課題とされている．
これらの課題に対し，近年ではブロックチェーンや分散型ストレージを活用した，
中央集権的管理に依存しないデータ管理手法が提案されている．

Aliら\cite{cite1}は，従来の中央集権的なIoTデータ管理における管理主体への依存やスケーラビリティの問題を指摘し，
...
また，IoTデバイス群ごとにサイドチェーンを構成することで，
単一のブロックチェーンにすべての処理を集中させない設計を採用しており，
分散的なIoTデータ管理におけるスケーラビリティ確保の重要性を示している．
一方で，評価実験ではブロックチェーン基盤の選択により計算負荷や通信オーバーヘッドが大きく異なることが報告されており，
データ登録や検証処理の配置がシステム性能に与える影響が示唆されている．

また，Krejciら\cite{cite2}は，IoTデータ配信におけるデータ完全性とリアルタイム性のトレードオフに着目し，
...
Raspberry Piを用いた評価実験の結果，
ブロックチェーンを用いた完全性検証は数百秒規模の遅延を伴うことが示されており，高頻度に実行される処理には適さない一方，
取引時など限定的なタイミングでの検証には有効であることが示されている．

これらに加え，ブロックチェーンをIoTへ適用する際のボトルネックとして，
コンセンサス処理に起因する遅延やスループット低下が指摘されており，
リソース制約下でも動作可能な軽量コンセンサスの導入による性能向上が検討されている．
Haqueら\cite{cite3}は，多数のIoTデバイスが存在する環境を想定し，
Delegated Proof of Stake（DPoS）を用いた軽量コンセンサスとIPFSを組み合わせたスケーラブルなIoTデータ管理フレームワークを提案している．
提案手法では，データ本体をIPFSへ保存し，ブロックチェーンには参照情報としてハッシュ値等のみを記録することで，
オンチェーン負荷を抑えつつ分散的なデータ管理を実現している．
また，ネットワーク性能をスループット，レイテンシ，リソース利用率などの観点から評価し，
DPoSがProof of Stake（PoS）と比較して低レイテンシかつ高い処理性能を示すことを報告している．

さらにHaqueら\cite{cite4}は，上記の方向性を発展させ，
DPoSに加えてシャーディング技術を導入することで並列処理能力を高め，
IoTネットワークにおける性能とスケーラビリティを向上させる手法を提案している．
提案手法は，トランザクション処理を分割し並列に検証することでスループット向上を狙い，
IPFSを用いたオフチェーン保存とオンチェーンでの参照情報管理を維持しつつ，
負荷増大時の処理時間やスループットの観点で改善を示している．
このように，IoTデータを分散ストレージへ格納し，ブロックチェーンには参照情報や検証情報のみを記録する設計に加えて，
コンセンサスの軽量化や並列化により性能課題へ対処する研究が進められている．

以上の先行研究から，IoTデータ管理においては，データ本体を分散ストレージに保存し，
ブロックチェーンはデータの参照情報や検証情報の管理に限定して用いることが，
スケーラビリティと信頼性の両立に有効であることが分かる．
一方で，データの所有者や提供者の正当性，すなわち「誰のデータであるか」や「正当な主体が提示しているか」といった主体に関する検証は，
分散環境における主体識別と検証の標準化まで含めて統合的に扱う研究は十分ではない．

本研究では，これらの知見を踏まえ，DIDを統合した分散型IoTデータ管理システムの構築を目指す．
DIDにより分散環境における主体識別を明確化し，IoTデータの真正性を第三者が検証可能とすることで，
既存研究では十分に扱われてこなかった主体の正当性検証を含むデータ管理を実現する点に本研究の特徴がある．
