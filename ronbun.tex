...
本研究で構築したシステムについて,各処理に要する時間およびスループットを測定し,性能評価を行った.
測定はすべて,各処理を20回繰り返した際の平均値を算出した.

まず,IoTデータのIPFSへのアップロード性能を評価した.
データ数10件(931bytes)の場合,アップロードに要する平均時間は19.11ms,平均スループットは0.054MB/sであった.
同様に,データ数100件(9\,279bytes)では平均時間20.78ms,平均スループット0.720MB/s,データ数1\,000件(96\,793bytes)
では平均時間21.20ms,平均スループット7.344MB/sを記録した.

次に,ブロックチェーン関連およびVC処理の性能評価を行った.
ブロックチェーンへの登録処理では,データ内容に依存しない条件下で,平均処理時間は63.30msであった.
また,VCの発行処理に要する平均時間は10.45ms,VCの検証処理に要する平均時間は155.782msであった.

以上の結果を表\ref{tab:performance_results}にまとめる.
...
\begin{table}[h]
  ...
  \begin{tabular}{l|l|r}
    ...
    IPFSスループット & 10件 & 0.054MB/s \\
    IPFSスループット & 100件 & 0.720MB/s \\
    IPFSスループット & 1\,000件 & 7.344MB/s \\
    ブロックチェーン登録 & --- & 63.60ms \\
    VC発行 & --- & 10.45ms \\
    VC検証 & --- & 155.782ms \\
    \hline
